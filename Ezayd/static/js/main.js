...
class EzaydApp {
  ...
  sortAuctions(sortType) {
    ...
    auctions.sort((a, b) => {
      switch (sortType) {
        case "Bientôt fini":
          // Sort by time remaining (you'd need to extract this from the DOM)
          return this.getTimeRemaining(a) - this.getTimeRemaining(b)
        case "Récemment ajouté":
          // Sort by date added (you'd need this data)
  ...
  getTimeRemaining(auction) {
    // Extract time remaining from auction element
  ...
  getDateAdded(auction) {
    // This would typically come from a data attribute
    return new Date(auction.dataset.dateAdded || Date.now())
  }

  getAuctionYear(auction) {
    // Extract year from auction title
